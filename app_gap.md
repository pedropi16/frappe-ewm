...
### RF UI polish + Consolidation Group — complete

Not part of the original SAP-parity phased roadmap — a follow-up pass requested directly against the live app: fix concrete RF usability gaps a fork audit found by reading the whole RF scanner app against real SAP EWM RF UX conventions, and add a genuinely new capability (Consolidation Group) that has no SAP-parity tracking above because it isn't porting an existing gap, it's new ground.

**RF UI fixes** (`frappe_wms/www/wms/index.html`), all grounded in real code reading, not speculation:
- The global Enter-key handler mapped every Enter keystroke straight to the screen's primary action — fine for single-field wizards, but broken on every multi-line screen (Receive/Deconsolidate/Ship/Count Detail): a scanner's terminating Enter after *any one* field tried to submit the whole screen with other lines still empty. Fixed by routing Enter within a `.line-row` to the next input instead, reserving "submit" for an explicit F8/button tap.
- Pick denial's revised-quantity path (P2) was unreachable from RF — `list_exception_codes` didn't return `allows_quantity_change` and the RF exception form never collected a quantity. Both fixed; a matching exception code now opens a quantity-entry step before submitting.
- A count held for recount/approval (P3's tolerance gating) was reported to the operator as "posted" regardless — `post_count`'s return status is now inspected instead of assumed.
- Autofocus added to the first field of every multi-line screen, matching the "walk up and scan" flow the single-field screens already had.
- Cluster-pick tasks (multiple `Stock Allocation`s bundled onto one task) now show a per-order breakdown in the RF task detail, not just the aggregate quantity — `api/scanner.py::get_task` resolves each allocation's Outbound Delivery for this.
- `api()`'s error handling now distinguishes a raw network failure (clear retry message) and a 401 (session expired — redirects to `/login?redirect-to=/wms`, the same path `www/wms/index.py`'s own Guest check already uses) from an ordinary server error.

**Consolidation Group**: the app already has Deconsolidation (splitting one mixed HU across several destinations); this adds the mirror — combining demand from several Outbound Deliveries *and* Work Orders onto one physical HU at a shared staging point (e.g. a Production Supply Area bin, or a distinct rack), then splitting it apart later. New doctypes `Consolidation Group` + `Consolidation Group Line` (module `WMS Execution`), new `services/consolidation.py`, new `"Consolidation"` `Warehouse Task`/`Warehouse Process Type` activity, new RF screen (list → add lines by scanning an Outbound Delivery/Work Order/Stock Allocation/Warehouse Request barcode → Gather → Split to Destinations).

- **Gather-then-split, not redirect-at-creation.** The obvious design — redirect a Pick or PSA-replenishment task's destination at creation time — turned out riskier than it looked: `_create_replenishment_request` has no `destination_hu` parameter, and `create_tasks_for_request`'s `remaining <= 0` guard makes "redirect an already-created task" real bookkeeping, not a param tweak. Picking and PSA replenishment run to completion completely unmodified; a **Consolidation** task (the exact mirror of `create_deconsolidation_tasks`, reversed) then gathers already-confirmed stock from wherever it normally landed onto the shared target HU, costing one extra physical hop in exchange for zero regression surface on the two most heavily-tested paths in the app.
- **A shared `batch_key` doesn't give you a queryable group of tasks** — it's never stored on `Warehouse Task`, only used transiently to find/reuse a `Warehouse Order`, and a deconsolidation run's tasks land in *different* Warehouse Orders once their destinations differ. `Warehouse Task` gained an actual `consolidation_group_line` Link field as the real anchor.
- **A picked Stock Allocation's `storage_bin`/`handling_unit` are a pre-pick snapshot, not where the stock is now** — a real bug caught only by running the test against the live bench: gathering from `allocation.storage_bin` failed with "Insufficient stock" because the Pick task had already moved that stock to the delivery's staging bin. Fixed to resolve the delivery's staging bin as the current location.
- **A production-supply Warehouse Request's `destination_hu` is never set** (`create_tasks_for_request` only resolves one when `destination_bin` is absent, and PSA staging always supplies it) — the physical HU carried unchanged through the Putaway confirmation is `request.source_hu`, not `destination_hu`. Another live-bench-caught bug of the same shape as P4's Kitting HU-dimension bug.
- Gathering material already sitting at its own PSA bin *onto a target HU in that same bin* trips the existing, correct `"Source and destination bins must differ"` validation (Consolidation isn't `Posting Change`). Not a bug — the Consolidation Group's own staging bin has to be a genuinely distinct physical point, which is realistic anyway (a rack elsewhere, not the PSA itself).

Verified with a new `test_consolidation.py` (4 tests: gather-then-split across a delivery and a Work Order end to end, `find_joinable_references` excludes already-joined lines, a line can't be removed once gathered, cancelling a delivery before it's picked clears its Consolidation Group Line) plus the full suite staying green — 246 tests passing, up from 242, zero regressions since `_create_pick_task_for_group`/`_create_replenishment_request`/`events/work_order.py` were never touched.

### Activity Area + Resource Group-owned queues + mass bin assignment — complete

Also not part of the phased roadmap — the user compared this app's RF logon flow directly against real SAP EWM behavior (log on to a Resource → queues come from your Resource Group) and flagged two things. Both turned out to be real, not just SAP-parity nice-to-haves:

- **Activity Area never existed** (confirmed by grep: zero references anywhere) — `determine_queue` could only route by whole Storage Type, not the finer-grained arbitrary bin grouping SAP EWM uses for queue determination and walking sequence.
- **`WMS Resource.resource_group` and `Warehouse Queue.resource_group` were already in the schema — and already 100% decorative.** Grepping every service/API file showed `resource_group` was only ever *read* for display (`list_available_resources`, `resource_workload`, `search_queues`), never matched against anything. The real (and only) queue-assignment mechanism was `WMS Resource.current_queue`, manually set one-at-a-time via `join_queue` — the exact same "existing, unused fields" pattern this project has found and fixed in every prior phase (P0's Exception Code fields, P2's pick-denial fields), just never caught before because nothing exercised it.

New **Activity Area** doctype (`wms_core` module, mirrors Storage Type/Section/Bin Type exactly - simple Link fields, no child tables, so it stays bulk-edit-friendly). `Storage Bin` and `Warehouse Queue` both gained an `activity_area` field; `determine_queue` extended to a 3-tier narrowest-wins match (activity area → storage type → blank fallback), same idiom as every other rule table in this app.

**Resource Group made real, not rebuilt.** `current_queue` stays as a genuine, useful "focus on one queue right now" mechanic - the fix was making Resource Group the actual *source of eligibility* underneath it: `_resource_for_queue` now falls back to any active, unfocused resource in the queue's own Resource Group when no one has explicitly joined it; `pull_next_warehouse_order` pulls from every queue in a resource's group when unjoined rather than hard-requiring one join first; `list_queues`/`join_queue` scope to the resource's own group. Every fallback is skipped cleanly when either side has no group configured, so a site that's never touched Resource Groups keeps working exactly as before - confirmed by the full suite staying green with zero changes needed to the one pre-existing test (`test_labor_management.py`) that sets `current_queue` directly.

**Mass bin assignment**: a new WMS Monitor "Bin Assignment" tab (filter bins by storage type/section/aisle/rack → select → bulk-apply one Activity Area), the actual "SAP mass transaction" the user asked for rather than relying on Frappe's own easy-to-miss generic list-view bulk-edit. Same filter/select/bulk-action shape as Slotting's rearrangement-task generation - deliberately reusable for other bulk bin edits later without new infrastructure.
...
### Correction: Warehouse Orders never auto-assign; real Blocked/On Hold lifecycle; RF Picking Auto/Manual submenu — complete

The immediately preceding pass (above) wired `WMS Resource.resource_group`/`Warehouse Queue.resource_group` into `_resource_for_queue`, which auto-assigned an idle grouped resource to a Warehouse Order **at creation time**. The user caught this as a real design mistake, not a nice-to-have: "resources only execute things, and they can get work 'assigned' but not by default... when warehouse orders are created there is no auto assign." Corrected in this pass:

- **`_resource_for_queue` deleted.** `get_or_create_warehouse_order` now always creates `status="Open"`, `assigned_resource=None` — a Warehouse Order sits queue-scoped only, regardless of whether its queue's Resource Group has an idle resource.
- **Claiming happens at two explicit points, never at creation.** *Auto*: `pull_next_warehouse_order` (unchanged from the prior pass) is now the only path that assigns proactively, and only when a resource explicitly asks for work. *Manual*: a new `_claim_warehouse_order_if_unassigned`, called from `confirm_task` right before the scan-verification checks, claims an unassigned WO for whoever confirms the first task on it — covers a task found via a manual search rather than a pull.
- **`list_my_tasks` closed a leftover gap.** It was still keyed off manual `current_queue` join only, even though `list_my_warehouse_orders`/`pull_next_warehouse_order`/`list_queues`/`join_queue` were already fixed to respect Resource Group eligibility in the prior pass — an unassigned, queued task is now visible to any resource whose Resource Group covers that queue, not just one that manually joined it.
- **Warehouse Order gained real `Blocked`/`On Hold` statuses** (`Open → Assigned → In Process → Blocked/On Hold → Completed/Cancelled`, plus a `blocking_reason` field mirroring `Warehouse Task`'s), matching the SAP EWM split the user asked to confirm explicitly: **Blocked** is automatic — `sync_warehouse_order` now also checks the WO's lead (lowest-sequence, non-terminal) task, and sets `Blocked` when it's in `Exception`, clearing back to `In Process`/`Open` once resolved. **On Hold** is a deliberate Supervisor pause (`block_warehouse_order`/`resume_warehouse_order`, desk-form buttons) — `sync_warehouse_order` returns early whenever a WO is already `On Hold` so the automatic recompute can never silently clear a deliberate pause.
- **RF Picking restructured into an Auto/Manual submenu** — the user asked for this pattern generically, planning to reuse it for other RF menus later, so a new `renderSubmenu(items)` helper renders any `{icon, label, action}` tile list rather than being built one-off for Picking. Auto pulls the next task (today's "Get Work"); Manual opens six labeled quick-picks (HU, Warehouse Task, Warehouse Order, Warehouse Request, Queue, Outbound Delivery) that all funnel into the same, now-widened `find_pick_tasks` auto-detect-by-existence lookup — the labels are a discoverability layer, not six separate code paths.

One existing test (`test_labor_management.py`) had baked in the now-reverted mistake — it asserted `task.assigned_resource == "LABOR-TEST-RESOURCE"` immediately after `attach_task`+`insert()`, before any confirm. Fixed to assert the task starts unassigned and only gets claimed once `confirm_task` runs (its resource fixture also needed a `user` link, since claiming now resolves "who confirmed it" from the acting session user, not from `current_queue` alone). `test_activity_area_queues.py`'s `test_resource_group_fallback_when_no_current_queue_joined` had the same baked-in assumption — rewritten to assert the task stays unassigned and is instead discoverable via `list_my_tasks` for a resource whose Resource Group covers the queue.

Verified with a new `test_warehouse_order_lifecycle.py` (5 tests: a fresh WO is `Open`/unassigned even with an idle resource logged into its queue; `pull_next_warehouse_order` still claims it; confirming a task claims an unassigned WO for the confirming resource; a WO becomes `Blocked` on its lead task's Exception and clears once resolved; `block_warehouse_order`/`resume_warehouse_order` require Supervisor and a Supervisor's hold survives an automatic `sync_warehouse_order` recompute) plus the two corrected tests above, plus the full suite — 258 tests passing, up from 253, zero regressions. RF Picking → Auto and Picking → Manual → (any of the six search types) both walked manually against the live bench to confirm they still reach a task.

### Fix: `depends_on: 1` crashed client-side rendering of mandatory fields on 4 doctypes

The user reported a real, recurring pattern found by hand across desk forms — not caught by any of this project's automated tests, since every test builds a doc as a fully-populated dict and `insert()`s it directly, which never touches client-side JS at all: mandatory fields (Storage Bin's `bin_code`, WMS Product's `item`, and others) sometimes failed to render or stayed uneditable, only surfacing as "field is mandatory" at Save time.

Root cause, confirmed by reading Frappe core's `evaluate_depends_on_value` (`frappe/public/js/frappe/form/layout.js`): 10 fields across 4 doctypes (`Storage Bin.bin_code`, `WMS Product.item`, `WMS Stock Balance.warehouse/product/handling_unit/storage_bin`, `WMS Stock Ledger Entry.warehouse/product/handling_unit/storage_bin`) had `"depends_on": 1` in their JSON — a bare integer, not a valid expression string or boolean. Frappe's evaluator only special-cases `typeof expression === "boolean"` before falling through to `expression.substr(0, 5)`, and **numbers have no `.substr` method** — this throws an uncaught `TypeError` during form layout, leaving the field's visibility toggle broken. None of these fields had any actual conditional-visibility requirement (they're all plain, always-relevant top-level fields), so the fix is simply deleting the bogus key rather than replacing it with a real expression.

Found via a static audit script (not a live browser, which isn't available in this environment) that parses every doctype JSON in the app and flags: non-string `depends_on`/`mandatory_depends_on`/`read_only_depends_on` values, `fetch_from` references to a nonexistent or non-Link field, always-hidden-and-mandatory fields with no default, and `.js` controller calls (`set_query`/`toggle_display`/`toggle_reqd`/etc.) referencing a fieldname that doesn't exist on that doctype — the last two categories came back clean. Verified via `bench console`: `frappe.get_meta(dt).get_field(fn).depends_on` now reads `None` on all 10 fields, confirmed by `git grep` that no other doctype has the same integer-value bug, and the full suite stayed green (258 tests) since this was always a pure client-rendering bug invisible to server-side `insert()`-based tests — exactly the gap the user was pointing at.

### Fix: duplicate desktop icon ("WMS" + "Frappe WMS"), and a real production asset-serving bug found along the way

The user reported seeing two tiles on the ERPNext desk for this app — one labeled "WMS", one "Frappe WMS" — with the icon not rendering on either.
